...
<head>
    ...
    <style>
        body { background: #05070b; color: #f1f5f9; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 15px; box-sizing: border-box; }
        .login-card { background: #131d31; border: 1px solid #1e293b; padding: 35px 25px; border-radius: 20px; width: 100%; max-width: 340px; box-shadow: 0 20px 40px rgba(0,0,0,0.7), 0 0 20px rgba(59, 130, 246, 0.1); text-align: center; box-sizing: border-box; }
...
<head>
    ...
    <style>
        ...
        .tabs { display: grid; grid-template-columns: repeat(3, 1fr); gap: 5px; background: #0b0f19; padding: 5px; border-radius: 10px; border: 1px solid var(--border); margin-bottom: 15px; }
        ...
        <div class="card">
            ...
            <div class="tab-content __CONTENT_WL_ACTIVE__">
                <form action="/admin/whitelist-ip" method="POST">
                    <div style="font-size: 13px; margin-bottom: 5px;">IP zur Whitelist hinzufügen:</div>
                    <div style="display: flex; gap: 10px;">
                ...
                <div style="font-size: 13px; margin: 15px 0 8px 0;">Whitelisted IPs:</div>
                <div style="max-height: 200px; overflow-y: auto;">
                    __WHITELIST_LIST__
                </div>
            </div>

            <!-- TAB 5: SICHERHEIT & DDOS-SCHUTZ (Vollständig integriert) -->
                ...
                <form action="/admin/update-settings" method="POST">
                    <div style="background: #0b0f19; padding: 15px; border-radius: 12px; border: 1px solid var(--border);">
                        ...
                        <div style="margin-bottom: 15px;">
                            ...
                            <input type="number" name="max_ip_req" value="__MAX_IP_REQ__" placeholder="Max Anfragen pro Sek." required>
                            <input type="number" step="0.1" name="throttle_delay" value="__THROTTLE_DELAY__" placeholder="Verzögerung in Sek." required>
                        </div>

                        <div style="margin-bottom: 15px;">
                            <label style="font-size: 11px; color: var(--text-muted); display:block; margin-bottom: 6px;">GLOBALES SERVER-LIMIT</label>
                            <input type="number" name="server_limit" value="__SERVER_LIMIT__" placeholder="Max. RPS Schwelle" required>
                        </div>
