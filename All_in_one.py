        ...
        function fetchData() {
            fetch('/api/stats').then(res => res.json()).then(data => {
                ...
                if(data.settings.autoban) {
                    ...
                    btnA.className = "btn" style="background:#475569;";
...
class TrafficHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # Alte Timestamps entfernen (1 Sekunde Fenster)
        ...
        # IP Ratenbegrenzung & Auto-Ban Logik
        if client_ip not in WHITELISTED_IPS:
            ...
            # Request count pro IP im Zeitfenster
        ...
        # Normale Besucherseite oder Wartungsmodus
        geo_loc, country = get_geoip_and_country(client_ip)
        country_stats[country] += 1
        status_code_stats[200] += 1
