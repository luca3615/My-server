...
<head>
    ...
    <style>
        ...
        .tabs { display: grid; grid-template-columns: repeat(3, 1fr); gap: 5px; background: #0b0f19; padding: 5px; border-radius: 10px; border: 1px solid var(--border); margin-bottom: 10px; }
        .tab { padding: 8px 4px; text-align: center; font-size: 11px; font-weight: 500; color: var(--text-muted); background: transparent; border: none; border-radius: 6px; cursor: pointer; text-decoration: none; transition: 0.2s; display: block; }
        .tab.active { background: var(--primary); color: #fff; font-weight: bold; }
        
        .btn-sec-link { background: #475569; color: white; padding: 10px; text-align: center; border-radius: 8px; margin-bottom: 15px; display: block; text-decoration: none; font-weight: bold; font-size: 12px; transition: background 0.2s; }
        .btn-sec-link:hover { background: #334155; }
        .btn-sec-link.active { background: var(--primary); }
...
<body>
    <div class="container">
        <div class="card">
            ...
            <!-- Haupt-Tabs (3er Raster) -->
            <div class="tabs">
                <a href="/admin?tab=logs" class="tab __TAB_LOGS_ACTIVE__">Logs</a>
            ...
            <!-- Separater Button für Sicherheit & Whitelist -->
            ...
            <!-- TAB 3: SICHERHEIT & WL (Kombiniertes Menü) -->
            <div class="tab-content __CONTENT_SEC_ACTIVE__">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                    <!-- Sperren-Sektion -->
                    <div style="background: #0b0f19; padding: 12px; border-radius: 12px; border: 1px solid var(--border);">
                        <form action="/admin/ban-ip" method="POST">
                            <div style="font-size: 12px; font-weight: bold; margin-bottom: 6px;">IP sperren:</div>
                            <input type="text" name="ip" placeholder="z.B. 192.168.1.50" required style="margin-bottom: 6px;">
                            <button type="submit" class="btn btn-danger" style="width: 100%; padding: 6px;">Sperren</button>
                        </form>
                        <div style="font-size: 12px; font-weight: bold; margin: 10px 0 6px 0;">Gesperrte IPs:</div>
                    ...
                    <div style="background: #0b0f19; padding: 12px; border-radius: 12px; border: 1px solid var(--border);">
                        <form action="/admin/whitelist-ip" method="POST">
                            ...
                            <input type="text" name="ip" placeholder="z.B. 192.168.1.100" required style="margin-bottom: 6px;">
                            <button type="submit" class="btn btn-success" style="width: 100%; padding: 6px;">Erlauben</button>
                        </form>
                        <div style="font-size: 12px; font-weight: bold; margin: 10px 0 6px 0;">Whitelisted:</div>
                        <div style="max-height: 120px; overflow-y: auto;">
                            __WHITELIST_LIST__
                        </div>
                    </div>
                ...
                <form action="/admin/update-settings" method="POST">
                    <div style="background: #0b0f19; padding: 15px; border-radius: 12px; border: 1px solid var(--border);">
                        <div style="margin-bottom: 12px;">
                            <label style="font-size: 11px; color: var(--text-muted); display:block; margin-bottom: 6px;">SYSTEM-STEUERUNG</label>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                <button type="submit" name="toggle_maint" value="1" class="btn __MAINT_BTN_CLASS__">Wartung: __MAINT_TEXT__</button>
                                <button type="submit" name="toggle_autoban" value="1" class="btn __AUTOBAN_BTN_CLASS__">Auto-Ban: __AUTOBAN_TEXT__</button>
                            </div>
                        </div>

                        <div style="margin-bottom: 12px;">
                            <label style="font-size: 11px; color: var(--text-muted); display:block; margin-bottom: 6px;">DROSSELUNG & LIMITS (PRO IP)</label>
                            <input type="number" name="max_ip_req" value="__MAX_IP_REQ__" placeholder="Max Anfragen pro Sek." required style="margin-bottom:6px;">
                        ...
                        <div style="margin-bottom: 12px;">
                            <label style="font-size: 11px; color: var(--text-muted); display:block; margin-bottom: 6px;">GLOBALES SERVER-LIMIT</label>
                            <input type="number" name="server_limit" value="__SERVER_LIMIT__" placeholder="Max. RPS Schwelle" required style="margin-bottom:0;">
                        </div>

                        <button type="submit" class="btn btn-primary" style="width: 100%; padding: 10px;">Einstellungen speichern</button>
                    </div>
                </form>
            </div>
    ...
    def do_GET(self):
        ...
        if path == "/admin":
            ...
            if f"session={ADMIN_PASSWORD}" in cookie:
                ...
                # Extra Highlight für den Sicherheits-Button
        ...
        if client_ip not in WHITELISTED_IPS:
            ...
            if len(timestamps) > MAX_REQUESTS_PER_IP:
                if AUTO_BAN_ENABLED:
                    ...
                    save_settings()
                status_code_stats[403] += 1
                self.send_error(403, "Rate Limit Exceeded")
                return
...
if __name__ == "__main__":
    with socketserver.TCPServer(("", PORT), FastTrafficHandler) as httpd:
        print(f"High-Performance Server läuft auf Port {PORT}")
        httpd.serve_forever()
