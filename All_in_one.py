...
def get_geoip_and_country(ip):
    if len(REQUEST_TIMESTAMPS) > 40:  # Schutz-Modus bei hoher Last
        return "Schutz-Modus", "Anti-DDoS"
...
<head>
    ...
    <style>
        ...
        h1 { font-size: 22px; margin-top: 0; background: linear-gradient(to right, var(--primary), var(--accent)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; display: flex; align-items: center; justify-content: space-between; }
        p { color: var(--text-muted); font-size: 13px; line-height: 1.5; margin-bottom: 25px; }
        .status-pill { display: inline-flex; align-items: center; gap: 8px; background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); color: var(--success); padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: bold; }
    </style>
</head>
<body>
    <div class="container">
        <div class="card">
            <h1>Live Monitor <span class="status-pill">● Online</span></h1>
            <p>Dein Server ist durch das Anti-DDoS-System geschützt.</p>
        </div>
    </div>
</body>
</html>"""
...
if __name__ == "__main__":
    with socketserver.ThreadingHTTPServer(("", PORT), FastTrafficHandler) as httpd:
