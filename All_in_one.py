...
<head>
    ...
    <style>
        ...
        .nav-tabs { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; margin-bottom: 20px; background: #0f172a; padding: 4px; border-radius: 10px; }
        .tab-btn { background: transparent; color: var(--text-muted); border: none; padding: 8px; font-size: 12px; font-weight: 600; cursor: pointer; border-radius: 6px; text-align: center; }
    ...
    <div class="container">
        <div class="card">
            <div class="header-row">
                ...
                <span class="badge-aktiv">● System Online</span>
            </div>
            <div class="sub-info">
                <span id="server-clock">Uhrzeit: 00:00:00</span>
            ...
            <div style="margin-top: 15px; text-align: center;">
                <a href="/admin" style="color: var(--text-muted); font-size: 11px; text-decoration: underline;">Zur Admin-Steuerung</a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function fetchData() {
            fetch('/api/stats').then(res => res.json()).then(data => {
                ...
                document.getElementById('server-clock').innerText = "Uhrzeit: " + data.server_time;
...
<head>
    ...
    <style>
        ...
        .sub-info { font-size: 11px; color: var(--text-muted); margin-bottom: 15px; display: flex; justify-content: space-between; }
        .badge-aktiv { background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); color: var(--success); padding: 3px 10px; border-radius: 20px; font-size: 11px; }
        .nav-tabs { display: grid; grid-template-columns: repeat(4, 1fr); gap: 4px; margin-bottom: 20px; background: #0f172a; padding: 4px; border-radius: 10px; }
        .tab-btn { background: transparent; color: var(--text-muted); border: none; padding: 8px 2px; font-size: 11px; font-weight: 600; cursor: pointer; border-radius: 6px; text-align: center; }
        .tab-btn.active { background: var(--primary); color: white; }
        .tab-content { display: none; }
        .tab-content.active { display: block; }
        .btn { background: var(--primary); color: white; border: none; padding: 8px 14px; border-radius: 8px; font-weight: 600; cursor: pointer; font-size: 12px; width: 100%; text-align: center; display: inline-block; box-sizing: border-box; text-decoration: none;}
...
<body>
    <div class="container">
        <div class="card">
            ...
            <div class="nav-tabs">
                <button class="tab-btn active" onclick="switchTab('bans')">Sperren</button>
                ...
                <button class="tab-btn" onclick="switchTab('actions')">Aktionen</button>
            </div>
            ...
            <div id="tab-whitelist" class="tab-content">
                <p style="color:var(--text-muted); font-size:12px; margin-top:0;">IP zur Whitelist hinzufügen:</p>
                <div style="display:flex; gap:8px;">
                ...
                <p style="color:var(--text-muted); font-size:12px;">Whitelisted IPs:</p>
                <div id="whitelist-list-container" style="max-height: 180px; overflow-y: auto;"></div>
            </div>

            <!-- TAB 3: SICHERHEIT -->
            <div id="tab-security" class="tab-content">
                <p style="color:var(--text-muted); font-size:12px; margin-top:0;">Wartungsmodus:</p>
                <button id="btn-maint" class="btn btn-warning" onclick="toggleSetting('maintenance')">Wartung umschalten</button>
                <p style="color:var(--text-muted); font-size:12px; margin-top:15px;">Auto-Bot-Schutz:</p>
                <button id="btn-autoban" class="btn" onclick="toggleSetting('autoban')">Bot-Schutz umschalten</button>
                <p style="color:var(--text-muted); font-size:12px; margin-top:15px;">Max. Anfragen pro IP / Sek:</p>
                <input type="number" id="input-max-req" onchange="updateConfigNum('max_ip_req', this.value)">
                <p style="color:var(--text-muted); font-size:12px; margin-top:5px;">Server-Limit (RPS Schwelle):</p>
                <input type="number" id="input-limit" onchange="updateConfigNum('server_limit', this.value)">
            </div>

            <!-- TAB 4: AKTIONEN -->
            <div id="tab-actions" class="tab-content">
                <button class="btn" style="background:#1e293b; margin-bottom:10px;" onclick="clearLogs()">Live-Logs leeren</button>
                <button class="btn btn-danger" style="margin-bottom:15px;" onclick="resetStats()">Zähler & Peak Reset</button>
                <a href="/" class="btn" style="background:#3b82f6; margin-bottom:10px;">Zurück zum Live-Monitor</a>
                <a href="/logout" class="btn btn-danger" style="background:#1e293b;">Abmelden</a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function fetchData() {
            fetch('/api/stats').then(res => res.json()).then(data => {
                ...
                btnM.innerText = data.settings.maintenance ? "Wartung: AN" : "Wartung: AUS";

                let btnA = document.getElementById('btn-autoban');
                btnA.innerText = data.settings.autoban ? "Auto-Bot-Schutz: AN" : "Auto-Bot-Schutz: AUS";
        ...
        function clearLogs() { fetch('/api/action?do=clear_logs'); }
        function resetStats() { fetch('/api/action?do=reset_stats'); }
        function manualBan() {
